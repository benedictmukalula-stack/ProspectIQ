...
import {
  createWorkspaceAutonomousPolicy,
  summarizeWorkspacePolicy,
} from "../lib/policies/persistent-policy-engine"
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      workspaceId,
      summary: summarizeWorkspacePolicy(workspacePolicy),
      workspacePolicy,
    })
  } catch (error) {
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Autonomous policy generation failed",
      },
      { status: 500 }
    )
  }
