export function calculateStrategicReadiness(initiatives: any[]) {
  if (!initiatives.length) return 85

  const penalty = initiatives.reduce((sum, item) => {
    if (item.priority === "high") return sum + 18
    if (item.priority === "medium") return sum + 9
    return sum + 4
  }, 0)
...
export function buildStrategicPlanSummary(initiatives: any[]) {
  return {
