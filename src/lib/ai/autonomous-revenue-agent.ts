...
export async function runAutonomousRevenueAgent({
  workspaceId,
  contactId,
  supabase,
}: {
  ...
  try {
    ...
    if (
      decision.decision === "accelerate_followup" ||
      ...
      await supabase
        ...
        .update({
          next_send_at: new Date(
            Date.now() + decision.recommendedDelayDays * 24 * 60 * 60 * 1000
          ).toISOString(),
        })
        .eq("workspace_id", workspaceId)
        .eq("contact_id", contactId)
    ...
    if (
      decision.decision === "soft_nurture" ||
