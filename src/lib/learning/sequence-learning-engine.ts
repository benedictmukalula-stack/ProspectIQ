...
export type SequenceLearningRule = {
  rule: string
...
export function generateSequenceLearningRules(memory: StrategicMemory) {
  ...
  if ((summary.replyRate || 0) > 0) {
    rules.push({
      ...
      recommendation:
        "Stop automated sequences once replies are detected and hand off to manual sales follow-up.",
      generationImpact: {
        cadence: "accelerate",
        tone: "direct",
        ctaStrength: "strong",
  ...
  if ((summary.engagementRate || 0) >= 50) {
    rules.push({
      ...
      recommendation:
        "Use shorter, more decisive sequences because the workspace is already producing strong engagement.",
      generationImpact: {
        cadence: "accelerate",
        tone: "consultative",
        ctaStrength: "strong",
  ...
  if ((summary.failureRate || 0) > 10) {
    rules.push({
      ...
      recommendation:
        "Reduce sending pressure and prioritize email quality checks before generating aggressive outreach.",
      generationImpact: {
        cadence: "extend",
        tone: "light-touch",
        ctaStrength: "soft",
  ...
  if ((summary.warm || 0) > (summary.cold || 0)) {
    rules.push({
      ...
      recommendation:
        "Generate value-led nurture sequences designed to convert warm contacts into hot opportunities.",
      generationImpact: {
        cadence: "standard",
        tone: "educational",
        ctaStrength: "medium",
  ...
  if ((summary.hot || 0) > 0) {
    rules.push({
      ...
      appliesTo: "hot leads",
      recommendation:
        "Generate high-intent follow-ups with clear meeting CTAs and shorter delay windows.",
      generationImpact: {
        cadence: "accelerate",
        tone: "direct",
        ctaStrength: "strong",
  ...
  if (topIndustry && topIndustry !== "general market") {
    rules.push({
      ...
      recommendation:
        `Adapt future AI-generated sequences to reference ${topIndustry} workflows, pain points, and operational outcomes.`,
      generationImpact: {
        cadence: "standard",
        tone: "consultative",
        ctaStrength: "medium",
  ...
  if (rules.length === 0) {
    rules.push({
      ...
      recommendation:
        "Insufficient historical signal. Continue using standard three-step educational outreach.",
      generationImpact: {
        cadence: "standard",
        tone: "educational",
        ctaStrength: "soft",
