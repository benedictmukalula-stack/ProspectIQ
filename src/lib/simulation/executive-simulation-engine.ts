...
export function generateExecutiveSimulations(forecast: ForecastSummary) {
  const scenarios: Scenario[] = []

  scenarios.push({
    name: "Double Outbound Volume",
    description:
      "Models the impact of doubling outbound activity while keeping current engagement quality.",
    ...
    expectedOutcome:
      "Higher reach may increase pipeline volume, but engagement quality could decline slightly.",
    recommendation:
      "Scale outbound gradually and monitor reply/failure rates before doubling volume fully.",
  })

  scenarios.push({
    name: "Shorten Sequences to 3 Steps",
    description:
      "Models performance if all high-intent sequences use shorter, direct 3-step cadence.",
    ...
    expectedOutcome:
      "Shorter sequences may improve speed-to-response for warm and hot leads.",
    recommendation:
      "Apply 3-step direct sequences to hot, warm, and high-engagement vertical segments.",
  })

  scenarios.push({
    name: "Delivery Risk Spike",
    description:
      "Models a negative scenario where delivery failures rise due to provider or list quality issues.",
    ...
    expectedOutcome:
      "Revenue performance could drop materially if delivery quality weakens.",
    recommendation:
      "Activate protective throttling, validate addresses, and suppress bounced contacts immediately.",
  })

  scenarios.push({
    name: "Vertical Focus: Logistics",
    description:
      "Models the impact of concentrating AI copy, sequences, and benchmarks around the strongest vertical.",
    ...
    projectedFailureRate: clamp(forecast.failureRate),
    expectedOutcome:
      "Focused vertical messaging may increase relevance and conversion probability.",
    recommendation:
      "Increase logistics-specific messaging, case examples, and executive sequence personalization.",
  })

  scenarios.push({
    name: "Protective Conservative Mode",
    description:
      "Models a conservative operating posture designed to protect deliverability and reduce risk.",
    ...
    expectedOutcome:
      "Lower volume may reduce near-term pipeline growth but improve quality and delivery safety.",
    recommendation:
      "Use this mode only when bounce/failure signals increase or provider reputation is uncertain.",
  })
