export type ExecutiveInsight = {
  ...
  category: "growth" | "risk" | "operations" | "security" | "readiness"
  summary: string
...
export function analyzeExecutiveWorkspace({
  intelligence,
  pipelineRisks = [],
  engagementSignals = [],
  autonomousActions = [],
...
export function calculateExecutiveReadinessScore({
  intelligence,
  insights,
}: {
  intelligence: any
  ...
  if (summary.contacts > 0) score += 8
  if (summary.activeSequences > 0) score += 8
  if (summary.sentEmails > 0) score += 8
  if (summary.engagementEvents > 0) score += 8
  if (summary.aiWorkflows > 0) score += 8
  if (summary.teamMembers > 0) score += 5

  if (health.emailProvider === "mock_mode") score -= 12
  if (health.security === "review_needed") score -= 15
