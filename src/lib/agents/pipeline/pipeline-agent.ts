...
export function analyzePipelineRisk(intelligence: any): PipelineRisk[] {
  ...
  if ((summary.contacts || 0) > 0 && (summary.activeSequences || 0) === 0) {
    risks.push({
      ...
      recommendation:
        "Activate outbound sequences and enroll high-fit contacts.",
  ...
  if ((performance.replyRate || 0) < 5 && (summary.sentEmails || 0) > 0) {
    risks.push({
      ...
      recommendation:
        "Improve CTA quality, personalization, and targeting segmentation.",
  ...
  if ((summary.aiRuns || 0) === 0) {
    risks.push({
      ...
      recommendation:
        "Run AI lead scoring and outbound drafting workflows.",
  ...
  if ((summary.sentEmails || 0) === 0 && (summary.contacts || 0) > 10) {
    risks.push({
      ...
      severity: "high",
      reason:
        "CRM contains leads but no outbound execution activity exists.",
      recommendation:
        "Launch outbound campaigns to activate pipeline generation.",
