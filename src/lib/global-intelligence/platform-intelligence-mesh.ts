...
export function generatePlatformIntelligenceMesh({
  leads,
  companies,
  engagementEvents,
  queue,
  ...
  const industryBenchmarks = Array.from(industryCounts.entries())
    .map(([industry, companyCount]) => {
      ...
      const workspaceSet = new Set(
        industryCompanies.map((company) => company.workspace_id).filter(Boolean)
      )

      const relatedLeads = leads.filter((lead) =>
        lead.workspace_id ? workspaceSet.has(lead.workspace_id) : false
      )

      const hot = relatedLeads.filter(
        (lead) => lead.hot_lead || lead.lifecycle_stage === "hot"
      ).length

      const warm = relatedLeads.filter(
  ...
  if (replied > 0) {
    platformPatterns.push({
      ...
      recommendation:
        "Maintain reply-aware stopping and manual handoff as a global default.",
      impact: "high",
    })
  }
  ...
  if (clicked > 0) {
    platformPatterns.push({
      ...
      recommendation:
        "Use stronger CTA policies for clicked or high-intent segments.",
      impact: "medium",
    })
  }
  ...
  if (failed + bounced > 0) {
    platformPatterns.push({
      ...
      recommendation:
        "Activate protective throttling for affected workspaces and suppress bounced contacts.",
      impact: "high",
    })
  }
  ...
  if (industryBenchmarks[0]) {
    platformPatterns.push({
      ...
      recommendation:
        `Generate more ${industryBenchmarks[0].industry}-specific messaging and benchmarks.`,
      impact: "medium",
    })
  }
  ...
  if (platformPatterns.length === 0) {
    platformPatterns.push({
      ...
      recommendation:
        "Continue collecting workspace activity before applying platform-level optimization.",
      impact: "low",
    })
  }
