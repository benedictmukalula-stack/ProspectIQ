...
export function runMultiAgentRevenueSystem({ leads, queue, }) {
    const insights = [];
    ...
    const salesReady = leads.filter((lead) => lead.lifecycle_stage === "sales-ready");
    ...
    if (salesReady.length > 0 || hotLeads.length > 0) {
        insights.push({
            ...
            action: "Assign these leads to direct follow-up, generate personalized emails, or queue AI follow-ups.",
        });
    }
    else {
        insights.push({
            ...
            insight: "No hot or sales-ready leads are currently detected in the workspace.",
            action: "Continue running nurture campaigns and simulate/track engagement signals.",
        });
    }
    if (failed > 0 || pending > 10) {
        insights.push({
            ...
            action: "Review failed delivery records, provider settings, and queue processing cadence.",
        });
    }
    else {
        insights.push({
    ...
    if (warmLeads.length > 0) {
        insights.push({
            ...
            action: "Generate nurture sequences or use educational follow-up content for these contacts.",
        });
    }
    if (coldLeads.length > warmLeads.length && coldLeads.length > 0) {
        insights.push({
            ...
            insight: "Cold leads currently outnumber warm leads, suggesting room to improve targeting or messaging.",
            action: "Test stronger subject lines, narrower ICP criteria, and more relevant first-touch copy.",
        });
    }
    else {
        insights.push({
            ...
            insight: "Warm or high-intent lead volume is healthy relative to cold contacts.",
            action: "Continue monitoring lifecycle movement and optimize based on replies/clicks.",
        });
    }
    ...
    insights.push({
        ...
        action: "Use the command center to focus sales activity on the highest-scored contacts and strongest engagement signals.",
    });
    return {
        summary: {
