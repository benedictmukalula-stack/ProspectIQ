export function generateSelfImprovementDirectives(mesh) {
    ...
    if ((summary.failureRate || 0) > 10) {
        directives.push({
            ...
            priority: "critical",
            reason: "Platform failure rate is elevated. Sending should become more conservative until delivery health improves.",
            tuning: {
                cadence: "extend",
    ...
    if ((summary.replyRate || 0) > 5) {
        directives.push({
            ...
            priority: "high",
            reason: "Reply rate is strong enough to justify default reply stopping and manual escalation behavior.",
            tuning: {
    ...
    if ((summary.engagementRate || 0) >= 50) {
        directives.push({
            ...
            priority: "high",
            reason: "Platform engagement is strong. Shorter, more decisive sequences should be favored.",
            tuning: {
                sequenceLength: "short",
                tone: "consultative",
                ctaStrength: "strong",
    ...
    if (topIndustry && topIndustry.hotRate >= 50) {
        directives.push({
            ...
            priority: "medium",
            reason: `${topIndustry.industry} is currently outperforming other verticals with ${topIndustry.hotRate}% hot-rate signal.`,
            tuning: {
    ...
    if (directives.length === 0) {
        directives.push({
            ...
            priority: "low",
            reason: "Insufficient platform-level signal to tune autonomous behavior aggressively.",
            tuning: {
