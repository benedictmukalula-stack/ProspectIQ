...
export function generateRevenueForecast({ leads, queue, engagementEvents, }) {
    const hot = leads.filter((lead) => lead.hot_lead || lead.lifecycle_stage === "hot").length;
    ...
    const salesReady = leads.filter((lead) => lead.lifecycle_stage === "sales-ready").length;
    ...
    const weightedPipelineScore = salesReady * 40 + hot * 25 + warm * 10 + averageScore;
    const forecastCategory = failureRate > 10
        ? "at_risk"
        : weightedPipelineScore >= 80
            ...
            : weightedPipelineScore >= 40
                ? "developing"
                : "early";
    const conversionOutlook = forecastCategory === "strong"
