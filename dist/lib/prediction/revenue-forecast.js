...
export function predictRevenueSignals({ intelligence, graph, }) {
    ...
    if ((summary.activeSequences || 0) === 0)
        pipelineRiskScore += 15;
    if ((summary.aiRuns || 0) === 0)
        pipelineRiskScore += 10;
    let anomalyScore = 0;
    ...
    let forecastLabel = "stable";
    if (conversionProbability >= 70 && pipelineRiskScore < 40)
        forecastLabel = "strong";
    if (pipelineRiskScore >= 55)
        forecastLabel = "at_risk";
    if (pipelineRiskScore >= 75 || anomalyScore >= 70)
        forecastLabel = "critical";
    return {
