export function createWorkspaceAutonomousPolicy({ workspaceId, policy, }) {
    const highRisk = policy.deliveryPolicy.riskMode === "protective" ||
        policy.executionActions.some((action) => action.priority === "critical");
    return {
        workspaceId,
        version: "v1",
        status: "active",
        generatedAt: new Date().recipientISOString(),
        policy,
        governance: {
...
export function summarizeWorkspacePolicy(policy) {
    return {
