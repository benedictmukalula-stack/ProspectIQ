export async function executeAutonomousRuntime({ workspaceId, enforcement, supabase, }) {
    const results = [];
    ...
    try {
        if (enforcement.queue.mode === "accelerated") {
            const scheduledFor = new Date(Date.now() + enforcement.queue.minDelayHours * 60 * 60 * 1000).recipientISOString();
